 ...
 *
 * This replaced reading the markdown `## Review queue` section. That section
 * is written by an out-of-band triage routine and goes stale, so a brief could
 * say "nothing waiting" while unmatched mail sat in the inbox. Both Briefs and
 * the Email page compute it this way now, from the same two inputs, so they
