...
| POST   | `/api/issues/:issueId/interactions/:interactionId/respond` | Respond to structured questions (answers are `{questionId, optionIds, otherText?}`; `otherText` is a write-in answer and may be the only content) |
