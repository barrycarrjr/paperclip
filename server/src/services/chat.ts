...
function systemPromptFor(session: ChatSession, defaultCompanyId: string | null): string {
  const lines = [
    ...
    "When the user expresses a HIGH-LEVEL intent they want carried out across all or several of their companies (e.g. 'get every company's Google reviews replied to', 'chase all overdue invoices', 'tighten support response times everywhere'), use the `broadcast_directive` tool with a clear plain-language `intent` — do NOT create issues one company at a time. It hands the intent to each company's CEO agent, who decomposes it and delegates to the right sub-agent. After it runs, briefly report which companies' CEOs were reached and which were skipped and why (e.g. no CEO, no access). If the intent is truly about one company, use create_issue instead.",
