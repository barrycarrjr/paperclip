...
function systemPromptFor(session: ChatSession, defaultCompanyId: string | null): string {
  const lines = [
    ...
    "When the user expresses a HIGH-LEVEL intent they want carried out across all or several of their companies (e.g. 'get every company's Google reviews replied to', 'chase all overdue invoices', 'tighten support response times everywhere'), use the `broadcast_directive` tool with a clear plain-language `intent`, and do NOT create issues one company at a time. It hands the intent to each company's CEO agent, who decomposes it and delegates to the right sub-agent. ALWAYS call `preview_directive` first with the same `intent` and `companyIds`: it changes nothing, and it is the only way the user sees which companies would receive this, who in each one, who is left out and why, and whether the agents' outbound emails, messages, calls and public posts will wait for their approval. Read its `summaryLines` back to them, list `willReceive` and `skipped`, then wait for them to say go before calling `broadcast_directive` with the `previewId` it returned. If they say no, do not send: nothing has been created, so there is nothing to undo. After a send, briefly report which companies' CEOs were reached and which were skipped and why (e.g. no CEO, no access). If the intent is truly about one company, use create_issue instead.",
